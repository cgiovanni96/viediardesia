const zones = [
  {
    id: "santanna",
    name: "Sant'Anna",
    position: { latitude: 40, longitude: 9 },
    text: {
      it:
        "Hospitale che risale al medioevo (XII-XIII sec.) costruito per il ricovero dei viandanti che percorrevano la via Romana, che qui doveva sorpassare le Rocche, scoscese e pericolose. La posizione e la funzione è analoga alle Grazie, fra Chiavari e Zoagli, hospitale medievale oggi Santuario.\n\nPer raggiungere Sant’Anna (nominata come SIC, Sito di Importanza Comunitaria) una passeggiata sale da Cavi borgo, giunge ai ruderi ancora visibili, con resti che risalgono al XIV e al XVI  secolo, per scendere a Sestri Levante in una passeggiata di incomparabile bellezza.\n\nUna leggenda romantica racconta di Anna, fanciulla figlia primogenita di un conte di Lavagna che trascorreva le sue giornate a scrutare il mare dall’alto, speranzosa di rivedere il suo amato tornare dalla guerra. Ormai convinta che non sarebbe più tornato, decise di togliersi la vita buttandosi da quella sommità nel mare tempestoso. Quando il giovane tornò, non trovò ad aspettarlo la donna che amava e decise di seguirla togliendosi la vita nello stesso modo.",
      eng:
        "A medieval (12th-13th century) hospice built to provide shelter for wayfarers travelling along the Via Romana, which had to pass the steep and dangerous Rocche here. The position and function is similar to Le Grazie, between Chiavari and Zoagli, a medieval hospice now a sanctuary.\n\nTo reach Sant'Anna (nominated as a SCI, Site of Community Importance) a walk climbs from Cavi borgo, reaches the still visible ruins, with remains dating back to the 14th and 16th centuries, and descends to Sestri Levante in a walk of incomparable beauty.\n\nA romantic legend tells of Anna, the first-born daughter of a Count of Lavagna, who spent her days looking down on the sea from above, hoping to see her beloved return from the war. Convinced that he would never return, she decided to take her own life by jumping from the summit into the stormy sea. When the young man returned, he did not find the woman he loved waiting for him and decided to follow her, taking his own life in the same way.",
    },
  },
  {
    id: "santagiulia",
    name: "Santa Giulia di Centaura",
    position: { latitude: 40, longitude: 9 },
    text: {
      it:
        "E’ la località più antica ed è rinomata per la posizione che domina tutto il Golfo del Tigullio, ammirabile dal piazzale della Chiesa (l’attuale edificio risale al 1600, ma è probabilmente costruito sull’antica cappella).\n\nDi particolare pregio il pavimento a risseu (rissolato), mosaico di sassi marini policromi, del piazzale della Chiesa.\n\nIl nome Centaura deriva da Centuri, località del Capo Corso (sull’isola che nelle giornate limpide si può da qui ammirare, insieme a Capraia e Gorgona), dove una giovane cristiana di nome Giulia fu martirizzata e crocifissa. \n\nLa leggenda racconta che le spoglie giunsero sulla spiaggia e furono conservate come reliquie nella Chiesa fino all’VIII secolo, quando furono trasportate nel noto monastero di Brescia a Lei dedicato. La tradizione religiosa si è mantenuta fino a un secolo fa, con il rito dei ceri illuminati che erano posti in mare sia a Cavi sia a Centuri e che le correnti marine trasportano sulle rispettive rive.\n\nNell’ XI secolo Santa Giulia apparteneva all’Arcivescovo di Genova, poi passò sotto il controllo dei Signori locali. La rete di sentieri qui percorribili è fitta e compete in bellezza con altri celebri percorsi liguri.",
      eng:
        "This is the oldest locality and is renowned for its position overlooking the entire Tigullio Gulf, which can be admired from the church square (the current building dates back to 1600, but was probably built on the ancient chapel). \n\nThe risseu (rissolato) floor, a mosaic of polychrome marine stones, in the church square is particularly valuable.\n\nThe name Centaura derives from Centuri, a locality on the Cap Corse (on the island that on clear days can be admired from here, together with Capraia and Gorgona), where a young Christian woman named Giulia was martyred and crucified. \n\nLegend has it that the remains arrived on the beach and were kept as relics in the Church until the 8th century, when they were transported to the well-known monastery in Brescia dedicated to her. The religious tradition was maintained until a century ago, with the rite of the illuminated candles that were placed in the sea both in Cavi and Centuri and that the sea currents carried to their respective shores.\n\nIn the 11th century Santa Giulia belonged to the Archbishop of Genoa, then came under the control of the local lords. The network of footpaths that can be followed here is dense and competes in beauty with other famous Ligurian routes.",
    },
  },
  {
    id: "sorlana",
    name: "Sorlana",
    position: { latitude: 40, longitude: 9 },
    text: {
      it:
        "Sorlana (probabile toponimo prediale romano) è località già nominata nell’XI secolo come luogo di possessi ecclesiastici, gestita dai conti di Lavagna di cui segue le vicende storiche. \n\nL’antica Chiesa (di origine forse medievale, ma ricostruita nel 1600) sorge nei pressi di un castagneto, dove a metà di Agosto si svolge ogni anno una famosa sagra.",
      eng:
        "Sorlana (probably a predial Roman toponym) is a place already mentioned in the 11th century as a place of ecclesiastical possessions, managed by the Counts of Lavagna, whose historical events it follows. \n\n The ancient church (perhaps medieval in origin, but rebuilt in 1600) stands near a chestnut grove, where a famous festival is held every year in mid-August.",
    },
  },
  {
    id: "barassi",
    name: "Barassi",
    position: { latitude: 40, longitude: 9 },
    text: {
      it:
        "Barassi ha molte similitudini con Sorlana, ma si differenzia per la presenza dei monaci Olivetani di Sant’Eufemiano che ricostruirono la parrocchiale nel XVI secolo. \n\nCome per Sorlana, il territorio terrazzato è dedicato in prevalenza all’uliveto e in parte modesta a orti, nei versanti più esposti al sole. Il bosco ceduo occupa, invece, gli altri territori, a conferma di una comunità contadina che qui ha vissuto, traendo sostentamento dal lavoro di una difficile terra terrazzata.",
      eng:
        "Barassi has many similarities with Sorlana, but differs in the presence of the Olivetan monks of Sant'Eufemiano who rebuilt the parish church in the 16th century.\n\nAs in Sorlana, the terraced land is mainly dedicated to olive groves and a small part to vegetable gardens on the slopes most exposed to the sun. Coppice, on the other hand, occupies the other territories, confirming the existence of a farming community that lived here, drawing sustenance from the work of a difficult terraced land.",
    },
  },
  {
    id: "cerreto",
    name: "Cerreto",
    position: { latitude: 40, longitude: 9 },
    text: {
      it:
        "Il toponimo (fitonimo) è riferito a un terreno sul quale si trovavano i Cerri (Quercus cerris), alberi tipici dei boschi submediterranei.\n\nUn’antica cava di ardesia si trova in questo territorio, a conferma che il nome Cavi potrebbe derivare dalla presenza di filoni ardesiaci sfruttati in epoche lontanissime. Mano a mano che questi si esaurivano, i cavatori risalivano il crinale, proseguendo con le estrazioni che giunsero fino sul monte San Giacomo. Solo al termine del XIX secolo, quando ormai anche queste cave non furono più redditizie, fu scoperto il ricco filone che da qui s’inoltrava lungo la Valle Fontanabuona e che è tutt’oggi utilizzato.",
      eng:
        "The toponym (phytonym) refers to a piece of land on which Cerris (Quercus cerris), a typical tree of sub-Mediterranean woods, used to grow. \n\nAn ancient slate quarry is located in this area, confirming that the name Cavi may derive from the presence of slate veins exploited in ancient times. As they became exhausted, the quarrymen went up the ridge, continuing with the extraction that reached as far as Mount San Giacomo. It was only at the end of the 19th century, when even these quarries were no longer profitable, that the rich vein was discovered, which ran from here along the Fontanabuona Valley and is still used today.",
    },
  },
  {
    id: "santeufemiano",
    name: "Sant'Eufemiano",
    position: { latitude: 40, longitude: 9 },
    text: {
      it:
        "A Sant’Eufemiano era presente un eremitaggio già citato nel XIII secolo, ma ricostruito nel tardo 1400. La chiesetta è dunque monastica e benedettina e del Santo di origine orientale si conservano le reliquie nella Prioria di Carasco.\n\nIl sito è raggiungibile da Cavi Borgo con la strada carrozzabile per Barassi e con una breve deviazione poco prima dell’abitato. Alla chiesetta di Sant’Eufemiano si arriva attraverso un bel sentiero ombroso nel bosco.",
      eng:
        "At Sant'Eufemiano there was a hermitage already mentioned in the 13th century, but rebuilt in the late 1400s. The small church is therefore monastic and Benedictine and the relics of the Saint of oriental origin are preserved in the Priory of Carasco. \n\nThe site can be reached from Cavi Borgo along the road to Barassi and with a short diversions just before the village. The small church of Sant'Eufemiano can be reached via a nice shady path through the woods.",
    },
  },
  {
    id: "crocetta",
    name: "Crocetta",
    position: { latitude: 40, longitude: 9 },
    text: {
      it:
        "La località si trova sul crinale alle spalle della chiesa di Santa Giulia, da qui raggiungibile con un panoramico sentiero. \n\nIl toponimo indica l’antica presenza una croce da rogazione nel luogo dove si recavano le processioni, ma anche l’incrocio dei sentieri di crinale con quelli provenienti da Lavagna e da Cavi.",
      eng:
        "The locality is on the ridge behind the church of Santa Giulia and can be reached by a panoramic path. \n\nThe place name indicates the ancient presence of a rogation cross in the place where processions went, but also the crossing of the ridge paths with those coming from Lavagna and Cavi.",
    },
  },
];

export default zones;
